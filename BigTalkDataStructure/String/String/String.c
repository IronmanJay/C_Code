...
/*初始条件：串S和T存在*/
/*操作结果，若S>T，则返回值>0，若S=T，则返回值=0，若S<T，则返回值小于0*/
...
/*返回字串T在主串S中第pos个字符之后的位置，若不存在，则函数返回值为0,其中，T非空，1≤pos≤StrLength(S)*/
...
/*T为非空串，若主串S中第pos个字符之后存在于T相等的字串，则返回第一个这样的字串在S中的位置，否则返回0*/
int Index2(String S, String T, int pos)
{
	...
	if (pos > 0)
	{
		...
		while (i <= n - m + 1)
		{
			SubString(sub, S, i, m); /*取主串中第i个位置长度与T相等的字串给sub*/
...
Status Replace(String S, String T, String V)
{
	...
	do
	{
		i = Index(S, T, i); /*结果i为从上一个i之后找到的字串T的位置*/
		if (i) /*串S中存在串T*/
		{
