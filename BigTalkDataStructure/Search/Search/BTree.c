...
/*生成含信息（T，r，ap）的新的根节点&T，原T和ap为子树指针*/
...
void InsertBTree(BTree *T, int key, BTree q, int i)
{
	...
	if (!finished) /*T是空树（参数q初值为NULL），或根节点已分裂为结点*q和*ap*/
	{
		NewRoot(T, rx, ap); /*生成含信息（T，rx，ap）的新的根节点*T，原T和ap为子树指针*/
	}
}
...
int mainBTree(void)
{
	...
	int r[17] = { 22,16,41,58,8,11,12,16,17,22,23,31,41,52,58,59,61 };
	BTree T = NULL;
	Result s;
	int i;
	for (i = 0; i < 17; i++)
